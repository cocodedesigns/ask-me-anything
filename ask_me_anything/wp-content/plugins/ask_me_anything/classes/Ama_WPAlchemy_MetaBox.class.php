...
add_action('admin_head', array('Ama_WPAlchemy_MetaBox', '_global_head'));

add_action('admin_footer', array('Ama_WPAlchemy_MetaBox', '_global_foot'));
...
class Ama_WPAlchemy_MetaBox
{
	...
	function Ama_WPAlchemy_MetaBox($arr)
	{
	...
	function _init()
	{
		...
		if ( ! Ama_WPAlchemy_MetaBox::_is_post() AND ! Ama_WPAlchemy_MetaBox::_is_page()) return;
	 ...
	 * @see		_is_page()
	 */
	private static function _is_post()
	{
		if ('post' == Ama_WPAlchemy_MetaBox::_is_post_or_page())
		{
			return TRUE;
		}
	...
	{
		if ('page' == Ama_WPAlchemy_MetaBox::_is_post_or_page())
		{
			return TRUE;
		}
	 ...
	 */
	private static function _is_post_or_page()
	{
		$post_type = Ama_WPAlchemy_MetaBox::_get_current_post_type();
	...
	static function _get_current_post_type()
	{
		...
		if ( isset( $uri ) )
		{
			...
			{
				$post_id = Ama_WPAlchemy_MetaBox::_get_post_id();
		...
		jQuery(function($)
		{
			...
			{		
				...
				if (elem.attr('class') && elem.filter('[class*=dodelete]').length)
				{
					...
					if (the_name){
						//Remove all fields
						Apprise('<h2>Remove All Fields</h2>This action cannot be undone. Are you sure you want to do this?', settings);
					} else {
						elem.parents('.wpa_group').css('background','red').fadeOut(300, function(){
							$(this).remove();
						});
					}
	...
	function _save($post_id)
	{
		...
		Ama_WPAlchemy_MetaBox::clean($new_data);
	...
	static function clean(&$arr)
	{
		if (is_array($arr))
		{
			foreach ($arr as $i => $v)
			{
				if (is_array($arr[$i]))
				{
					Ama_WPAlchemy_MetaBox::clean($arr[$i]);
